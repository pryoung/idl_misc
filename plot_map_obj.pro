...
FUNCTION plot_map_obj, map, _extra=_extra, log=log, dmin=dmin, dmax=dmax, $
                       ...
                       velocity=velocity, contourr=contourr, $
                       rgb_table=rgb_table
...
;             if /CONTOUR is set, i.e., contours are plotted on
;             existing image. If you use /LOG, then LEVELS should be
;             set to values of the original image, not the log image.
;     Rgb_Table:  The number of the predefined IDL color table, or a
;                 3 x 256 or 256 x 3 byte array containing color values
;                 to use for vertex colors. I
;
; KEYWORDS:
;     Most keywords for the IDL routine IMAGE.PRO can be input. In
;     addition there are the following inputs.
...
;     CONTOUR: If set, then the image is displayed as a contour.
...
;        For /contour, I've adjusted the spatial locations
...
;     Ver. 5, 21-Oct-2021, Peter Young
;        I've introduced /contour to draw a contour plot, and
;        /overplot is now used to overplot the image on a previous
;        plot.
;     Ver. 6, 26-Apr-2022, Peter Young
;        I've changed /contour to /contourr, as the former results
...
IF n_params() LT 1 THEN BEGIN
  print,'Use:  IDL> p=plot_map_obj(map, [ dmin=, dmax=, /log, /reverse, /velocity, /contour'
  ...
  print,'   /contour - the map will be overplotted on an existing map as contours'
  return,-1
ENDIF
...
;
; For /contour, I had to adjust x and y as contours weren't
; appearing at exactly the location of the intensity feature. 
;
IF keyword_set(contourr) THEN BEGIN 
...
;if rgb_table is not set check to see if the rgb table is in the map structure and use that. 
;It will be written  over if keyword_set(velocity)
if not keyword_set(rgb_table) and (tag_exist(map,'red') and tag_exist(map,'green') and tag_exist(map,'blue')) then begin
  rgb_table=[[map.red],[map.green],[map.blue]]
endif 
